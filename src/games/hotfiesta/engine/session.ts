...
import { SCATTER_FREE_SPINS } from './symbols';
...
export interface HotFiestaSnapshot {
  phase: GamePhase;
  grid: Grid;
  balance: number;
  bet: number;
...
export class HotFiestaSession {
  ...
  spin(): void {
    ...
    if (scatterCount >= SCATTER_TRIGGER) {
      const award = SCATTER_FREE_SPINS[Math.min(scatterCount, 7)] ?? 10;
      this.pendingFreeSpins = true;
      this.freeSpinBet = this.bet;
      this.freeSpinsRemaining = award;
      this.freeSpinsTotal = award;
