/**
 * Quest Raider — optional PAR-driven reel strips + paytable (from extracted xlsx).
...
export const QUEST_RAIDER_USE_PAR_MATH = true;

/** `null` = follow `QUEST_RAIDER_USE_PAR_MATH`; only consulted in dev. */
...
export function isQuestParMathEnabled(): boolean {
  if (import.meta.env.DEV && devParMathOverride !== null) return devParMathOverride;
  return QUEST_RAIDER_USE_PAR_MATH;
}

/** Dev-only: force PAR strips/paytable on or off, or `null` to use `QUEST_RAIDER_USE_PAR_MATH` again. */
export function setQuestRaiderParMathDevOverride(value: boolean | null): void {
...
export function parCodeToSymbol(code: string): TempleSymbol {
  ...
  if (!s) {
    console.warn('[Quest Raider PAR] unknown symbol code:', code);
    return 'birdBlue';
  }
  return s;
}
