/**
 * Aztec — drop, highlight, pop, fall, particles (5×3 line-pay avalanche).
...
export const AZTEC_SPIN_FALL_IN_TIMING: FallAnimTimingOptions = {
  colStaggerMs: AZTEC_SPIN_REEL_COL_STAGGER_MS,
  rowStaggerMs: 2,
  indexStaggerMs: 0,
  stackBottomFirst: true,
  suppressDelayJitter: true,
 ...
 */
export function aztecSpinStripExplodeTiming(row: number): { delayMs: number; durationMs: number } {
  const delayMs = 0;
  const fallDistCells = ROWS - row + 1.2;
  const dist = Math.max(1, Math.round(fallDistCells));
  const durationMs = fluidFallDurationMs(AZTEC_SPIN_STRIP_DROP_BASE_MS, dist);
